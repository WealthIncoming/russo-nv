...
const CONTENT = {
  NL: {
    ...
    intro: [
      'Industriële schilderwerken draaien om bescherming, niet om esthetiek. Een correct opgebouwd coatingsysteem beschermt staal jarenlang tegen corrosie, chemicaliën, hitte en slijtage — en dat begint bij een vakkundige oppervlaktevoorbereiding. Russo NV verzorgt het volledige traject: van stralen en grondlagen tot de afwerkende toplaag, met meetbare kwaliteit op elke stap.',
    ...
    scope: [
      { title: 'Oppervlaktevoorbereiding & stralen', text: 'Stralen tot reinheidsgraad Sa 2½ of Sa 3 met het juiste ankerprofiel — de basis van elke duurzame coating.' },
    ...
    process: [
      ...
      { step: '05', title: 'Inspectie & oplevering', text: 'Laagdiktemeting, controle en rapportage — het bewijs dat de specificatie gehaald is.' },
    ],
    processLink: 'Lees meer: hoe bereid je staal voor op coating',
    sectorsTitle: 'Voor welke sectoren',
    sectors: ['Petrochemie & raffinaderijen', 'Tankterminals & opslag', 'Maritiem & haven', 'Staalconstructies', 'Chemie', 'Voedingsindustrie'],
    ...
    why: [
      ...
      { icon: ClipboardCheck, title: 'Bewezen referenties', text: 'Grootschalige projecten voor o.a. Sea Tank Terminal, GTS en CSP Zeebrugge.' },
    ],
  ...
  EN: {
    ...
    intro: [
      'Industrial painting is about protection, not aesthetics. A correctly built coating system protects steel for years against corrosion, chemicals, heat and wear — and that starts with expert surface preparation. Russo NV handles the whole process: from blasting and primers to the finishing top coat, with measurable quality at every step.',
    ...
    process: [
      ...
      { step: '05', title: 'Inspection & handover', text: 'Film-thickness measurement, checks and reporting — proof the specification was met.' },
    ],
    processLink: 'Read more: how to prepare steel for coating',
    sectorsTitle: 'Sectors we serve',
    sectors: ['Petrochemical & refineries', 'Tank terminals & storage', 'Marine & port', 'Steel structures', 'Chemical', 'Food production'],
    ...
    why: [
      ...
      { icon: ClipboardCheck, title: 'Proven references', text: 'Large-scale projects for clients including Sea Tank Terminal, GTS and CSP Zeebrugge.' },
    ],
    ...
    ctaText: 'Send us your specification or schedule a short inspection — you’ll get a clear quote for your industrial painting.',
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="relative w-full min-h-[60vh] flex items-end overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src={HERO_IMAGE} alt={`Russo NV — ${c.h1} ${locale === 'NL' ? 'op staal in de haven van Antwerpen' : 'on steel in the Port of Antwerp'}`} className="w-full h-full object-cover" width={1920} />
      ...
      <section className="w-full bg-foreground text-white">
        <div className="max-w-[100rem] mx-auto px-8 py-20 lg:py-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-white/10 border border-white/10">
            {c.sectors.map((s) => (
              <div key={s} className="bg-foreground p-8 flex items-center">
                <span className="font-heading text-lg sm:text-xl text-white">{s}</span>
              </div>
            ))}
          </div>
        </div>
